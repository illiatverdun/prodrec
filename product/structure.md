...
```mermaid
flowchart TD
    A["Завантаження — LOADING…"] --> B{"Дані з попередніх сесій на місці?"}
...
flowchart TD
    ...
    B --> C{"Є проєкт, до якого віднести день?"}
    C -->|ні| D["Додати проєкт"]
    D --> B
    C -->|так| E{"День, який треба відмітити, видно в сітці?"}

    E -->|ні| F["Приховування вихідних"]
    F --> F1{"Здогадалася, що день сховано прапорцем вихідних?"}
    F1 -->|так| B
    F1 -->|ні| Z1["ГЛУХИЙ КУТ: працювала в суботу, відмітити нікуди"]

    E -->|так| G["Вибір проєкту для дня — поповер"]
    G --> H{"День належить одному проєкту?"}
